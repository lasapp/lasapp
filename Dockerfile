...
RUN pip install z3-solver==4.12.2.0;

WORKDIR /LASAPP

COPY src/py/requirements.txt src/py/
RUN pip install --no-cache-dir -r src/py/requirements.txt;

COPY src/jl/Project.toml src/jl/
RUN julia --project=src/jl -e "using Pkg; Pkg.instantiate();"

COPY . .

# if built from windows
RUN dos2unix scripts/start_servers.sh scripts/stop_servers.sh scripts/test_servers.sh